    ...
    <div className="p-5 lg:p-6 max-w-2xl">
      ...
      <div className="bg-white border border-pp-border rounded-2xl overflow-hidden">
        {notifications.length === 0 ? (
          <div className="p-10 text-center text-[14px] font-bold text-pp-gray">All caught up! No notifications.</div>
        ) : notifications.map(n => (
          <div key={n.id} className={`flex items-start gap-3 px-4 py-4 border-b border-gray-50 last:border-0 ${!n.read?'bg-pp-green-light/20':''}`}>
            <div className={`w-2 h-2 rounded-full mt-2 shrink-0 ${n.read?'bg-transparent':'bg-pp-green'}`}/>
            <div className="flex-1 min-w-0">
              ...
              {n.link ? <Link href={n.link} className="mt-2 inline-flex text-[11px] font-extrabold text-pp-green hover:underline">Open related page</Link> : null}
            </div>
          </div>
        ))}
      </div>
