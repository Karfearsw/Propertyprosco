...
export default async function RealtorBillingPage() {
  ...
  const status = realtor?.subscriptionStatus ?? SubscriptionStatus.CHECKOUT_PENDING
  const canManageBilling = Boolean(realtor?.stripeCustomerId)
      ...
      <div className="mb-4 rounded-2xl border border-pp-border bg-white p-4">
        ...
        <div className="mt-1 text-[12px] font-bold text-pp-gray">
          {realtor?.stripeCustomerId
            ? 'Your Stripe billing profile is connected and ready for plan management.'
            : 'No payment method is on file yet. Add one to unlock client coordination tools.'}
        </div>
      </div>
        ...
        <div className="rounded-xl border border-pp-border bg-pp-bg p-6">
          <div className="mb-4 flex items-center gap-3">
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-white">
              <CreditCard size={24} className="text-pp-gold" />
            </div>
            <div>
              <div className="text-[14px] font-black text-pp-dark">{realtor?.stripeCustomerId ? 'Stripe billing profile on file' : 'No payment method on file yet'}</div>
              <div className="text-[12px] font-bold text-pp-gray">
                {realtor?.stripeCustomerId
                  ? 'Use the billing portal to update cards, review invoices, or cancel.'
                  : 'Use the secure Stripe setup flow to activate your subscription and unlock paid workspace access.'}
              </div>
            </div>
          </div>
          <div className="grid sm:grid-cols-2 gap-3 text-[12px] font-bold text-pp-gray">
            <div className="rounded-xl bg-white px-4 py-3">
              ...
              <div className="text-[13px] font-black text-pp-dark">{user.email ?? 'Not available'}</div>
            </div>
            <div className="rounded-xl bg-white px-4 py-3">
              <div className="text-[11px] uppercase tracking-[1px] text-pp-gray mb-1">Agency</div>
              <div className="text-[13px] font-black text-pp-dark">{realtor?.agencyName ?? 'Complete your profile'}</div>
            </div>
          </div>
        </div>
      </div>
