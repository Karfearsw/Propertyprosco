...
export default async function ProQuotesPage() {
  ...
  const statusLabel: Record<string,string> = { PENDING:'Pending', VIEWED:'Viewed', ACCEPTED:'Accepted', DECLINED:'Declined' }
  const statusStyle: Record<string,string> = { PENDING:'bg-pp-gold-light text-pp-gold', VIEWED:'bg-pp-blue-light text-pp-blue', ACCEPTED:'bg-pp-green-light text-pp-green', DECLINED:'bg-red-100 text-pp-red' }
  ...
  return (
    <div className="p-5 lg:p-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
        {[['Quotes Sent',stats.sent,'text-pp-dark'],['Pending',stats.pending,'text-pp-gold'],['Viewed',stats.viewed,'text-pp-blue'],['Accepted',stats.accepted,'text-pp-green']].map(([l,v,c]) => (
      ...
      <div className="bg-white border border-pp-border rounded-2xl overflow-hidden">
        ...
        {quotes.length === 0 ? (
          ...
          <div key={q.id} className="flex items-center gap-3 px-4 py-3.5 border-b border-gray-50 last:border-0 hover:bg-gray-50 transition-colors">
            ...
            <span className={`text-[10px] font-black px-2 py-0.5 rounded-full shrink-0 ${statusStyle[q.status]}`}>{statusLabel[q.status]}</span>
          </div>
        ))}
      </div>
    </div>
