...
export default function RealtorLayout({ children, user }: Props) {
  ...
  return (
    <div className="min-h-dvh overflow-x-clip bg-pp-bg font-body">
      <header className="sticky top-0 z-50 flex h-16 items-center justify-between border-b border-white/6 bg-re-sidebar px-4 sm:px-5">
      ...
      <div className={`fixed bottom-0 right-0 top-16 z-50 flex w-72 flex-col overflow-y-auto border-l border-white/7 bg-re-sidebar p-3 shadow-2xl transition-transform duration-200 lg:hidden ${drawerOpen ? 'translate-x-0' : 'translate-x-full pointer-events-none'}`}>
      ...
      <div className="flex">
        <aside className="sticky top-16 hidden h-[calc(100dvh-4rem)] w-[220px] shrink-0 overflow-y-auto bg-re-sidebar p-2.5 lg:flex">
